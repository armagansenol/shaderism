...
const fragmentShader = `
  ...
  vec3 ChromaticAberration(sampler2D tex, in vec2 uv, float distortionStrength) {
    float rChannel = texture2D(tex, PincushionDistortion(uv, 0.5 * distortionStrength)).r;
    float gChannel = texture2D(tex, PincushionDistortion(uv, 0.25 * distortionStrength)).g;
    float bChannel = texture2D(tex, PincushionDistortion(uv, 0.125 * distortionStrength)).b;
...
class PincushionChromaticAberrationEffect extends Effect {
  constructor({ intensity = 0.3, distortionAmount = 0.2, animationSpeed = 0.8 } = {}) {
    super("PincushionChromaticAberrationEffect", fragmentShader, {
...
interface PincushionChromaticAberrationParams {
  intensity?: number
  distortionAmount?: number
  animationSpeed?: number
}

const PincushionChromaticAberrationPostEffect = forwardRef<
  PincushionChromaticAberrationEffect,
  PincushionChromaticAberrationParams
>((props, ref) => {
  const effectRef = useRef<PincushionChromaticAberrationEffect>(null!)

  const { camera } = useThree()
  const effect = useMemo(() => new PincushionChromaticAberrationEffect(props), [props])
  ...
  // Leva controls for Pincushion Chromatic Aberration
  useControls("Pincushion Chromatic Aberration", {
    "Global Intensity": {
...
PincushionChromaticAberrationPostEffect.displayName = "PincushionChromaticAberrationPostEffect"

export { PincushionChromaticAberrationPostEffect, PincushionChromaticAberrationEffect }
